...
class MarketMaker:
    def __init__(
        ...
        self.params = None
        self.max_position = max(20, min(200, int(max(cash_balance, 1.0) / 20)))
    ...
    def on_trade(self, option: BinaryOption, price: float, quantity: int, counterparty_id: int) -> None:
        ...
        self.cash_balance = max(self.cash_balance, 0.0)
    ...
    @classmethod
    def _regression(cls, ys, xs):
        vx = cls._variance(xs)
        beta = cls._covariance(xs, ys) / vx if vx > 1e-14 else 0.0
    ...
    def warm_up(self, market_history: MarketHistory) -> None:
        ...
        if rate_changes:
            ...
            up0, up_slope = self._regression(up, tilts)
            down0, down_slope = self._regression(down, tilts)
            strength = self._clip((up_slope - down_slope) / 2.0, 0.0, 1.0)
            p_up = self._clip(up0, 0.001, 0.998)
            p_down = self._clip(down0, 0.001, 0.999 - p_up)
        else:
        ...
        ajr_drift, ajr_rate_beta = self._regression(ajr_ret, rate_changes)
        thr_drift, thr_rate_beta = self._regression(thr_ret, rate_changes)
        ...
        cov = self._covariance(ajr_resid, thr_resid)
        shared_var = min(abs(cov), va, vt)
        sector_sd = math.sqrt(max(shared_var, 0.0))
        ajr_sector_beta = 1.0
        thr_sector_beta = 1.0 if cov >= 0 else -1.0

        self.params = MarketParameters(
            ajarai_drift=ajr_drift,
            ajarai_idio_std_dev=math.sqrt(max(va - shared_var, 1e-12)),
            ...
            theriodic_idio_std_dev=math.sqrt(max(vt - shared_var, 1e-12)),
    ...
    def _reservation_and_half_spread(self, option):
        ...
        half = self._clip(0.012 + 0.018 * event_risk + 0.002 * time_factor + 0.025 * utilization, 0.015, 0.12)
