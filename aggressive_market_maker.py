...
class MarketMaker:
    def __init__(
        ...
        self.params = None
        self.max_position = 500  # Emergency guard only; premium-at-risk is binding.
        self.reserve_fraction = 0.20
        self.per_trade_fraction = 0.35
        self.underlying_fraction = 0.50
        self._long_quantity_by_option_id = defaultdict(int)
        self._short_quantity_by_option_id = defaultdict(int)
        self._collateral_by_option_id = defaultdict(float)
        self._collateral_by_underlying_id = defaultdict(float)
    ...
    def on_step_advance(
        ...
        for option in self.active_option_state:
            ...
            if expired:
                ...
                long_quantity = self._long_quantity_by_option_id.pop(option.option_id, 0)
                short_quantity = self._short_quantity_by_option_id.pop(option.option_id, 0)
                self.cash_balance += long_quantity * settlement + short_quantity * (1.0 - settlement)
                collateral = self._collateral_by_option_id.pop(option.option_id, 0.0)
                leg_ids = {leg.underlying_id for leg in option.legs}
                collateral_share = collateral / max(len(leg_ids), 1)
                for underlying_id in leg_ids:
                    self._collateral_by_underlying_id[underlying_id] = max(
                        0.0,
                        self._collateral_by_underlying_id[underlying_id] - collateral_share,
                    )
    ...
    def on_trade(self, option: BinaryOption, price: float, quantity: int, counterparty_id: int) -> None:
        ...
        if quantity > 0:
            collateral = quantity * price
            self._long_quantity_by_option_id[option.option_id] += quantity
        else:
            collateral = (-quantity) * (1.0 - price)
            self._short_quantity_by_option_id[option.option_id] += -quantity
        self.cash_balance -= collateral
        self._collateral_by_option_id[option.option_id] += collateral
        leg_ids = {leg.underlying_id for leg in option.legs}
        collateral_share = collateral / max(len(leg_ids), 1)
        for underlying_id in leg_ids:
            self._collateral_by_underlying_id[underlying_id] += collateral_share
    ...
    def _reservation_and_half_spread(self, option):
        ...
        position_risk = position * fair if position >= 0 else (-position) * (1.0 - fair)
        contract_budget = max(self.per_trade_fraction * self.initial_cash, 1e-9)
        utilization = self._clip(position_risk / contract_budget, 0.0, 1.0)
        skew_direction = 1.0 if position > 0 else (-1.0 if position < 0 else 0.0)
        skew = 0.05 * skew_direction * utilization * max(0.35, 4.0 * fair * (1.0 - fair))
    ...
    def _capital_budget(
        ...
        reserve = self.reserve_fraction * self.initial_cash
        deployable = max(0.0, self.cash_balance - reserve)
        fraction = self.per_trade_fraction if trade_fraction is None else trade_fraction
        trade_budget = min(fraction * self.cash_balance, deployable)
        if risk_per_contract <= 0:
            return trade_budget
        leg_ids = {leg.underlying_id for leg in option.legs}
        underlying_limit = self.underlying_fraction * self.initial_cash
        underlying_budget = min(
            max(0.0, underlying_limit - self._collateral_by_underlying_id[underlying_id])
            for underlying_id in leg_ids
        ) * max(len(leg_ids), 1)
        return max(0.0, min(trade_budget, underlying_budget))
    ...
    def quote(self, option: BinaryOption, counterparty_id: int) -> Quote:
        ...
        bid_budget = self._capital_budget(option, bid_risk)
        offer_budget = self._capital_budget(option, offer_risk)
        bid_capacity = int(bid_budget / max(bid_risk, 0.01))
        offer_capacity = int(offer_budget / max(offer_risk, 0.01))
        bid_qty = min(100, max(0, self.max_position - position), bid_capacity)
        offer_qty = min(100, max(0, self.max_position + position), offer_capacity)
        if bid_qty < 1:
            bid, bid_qty = 0.0, 1
        if offer_qty < 1:
    ...
    def respond_to_fok(self, option: BinaryOption, fok_order: FokOrder) -> bool:
        reservation, _ = self._reservation_and_half_spread(option)
        position = self.position.option_quantity_by_option_id[option.option_id]
        if fok_order.order_type == OrderType.BUY:
            projected = position - fok_order.quantity
            edge = fok_order.price - reservation
            risk_per_contract = 1.0 - fok_order.price
        else:
            projected = position + fok_order.quantity
            edge = reservation - fok_order.price
            risk_per_contract = fok_order.price
        capital_needed = fok_order.quantity * risk_per_contract
        capital_budget = self._capital_budget(option, risk_per_contract, trade_fraction=0.50)
        reduces_inventory = abs(projected) < abs(position)
        minimum_edge = 0.005 if reduces_inventory else 0.01
        minimum_return = 0.05 if reduces_inventory else 0.15
        return_on_risk = edge / max(risk_per_contract, 0.01)
        return (
            abs(projected) <= self.max_position
            and capital_needed <= capital_budget
            and edge >= minimum_edge
            and return_on_risk >= minimum_return
        )
